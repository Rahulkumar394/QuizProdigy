...
export class AdminService {
  ...
  getAllRequestTeacher() {
    ...
    return this.http.get('http://localhost:8080/admin/pending-teacher',{headers});
  }
  ...
  getAllAcceptedTeacher() {
    const headers=new HttpHeaders().set('Authorization','Bearer '+Cookie.get('token'));
    console.log("Getting teacher with headers", headers);
    return this.http.get('http://localhost:8080/admin/accepted-teacher',{headers});
  }
  ...
  approveTeacher(teacherId:String){
    ...
    return this.http.get('http://localhost:8080/admin/approve-request/'+teacherId,{headers});
  }                 
  ...
  rejectTeacher(teacherId:String){
    ...
    const headers=new HttpHeaders().set('Authorization','Bearer '+Cookie.get('token'));
    console.log("Getting teacher with headers", headers,teacherId);
    return this.http.get('http://localhost:8080/admin/reject-request/'+teacherId,{headers});
  }

  getStudents() {
    const headers=new HttpHeaders().set('Authorization','Bearer '+Cookie.get('token'));
    console.log("Getting students with headers", headers);
    return this.http.get('http://localhost:8080/admin/accepted-student',{headers});
  }
}
